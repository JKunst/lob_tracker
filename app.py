...
def logout():
    for key in ["logged_in", "user", "role", "current_task"]:
...
if not st.session_state.logged_in:
    st.title("Loopbaanoriëntatie — Havo 4")
    st.write("Log in om aan de slag te gaan met je loopbaanoriëntatie.")
    st.info('Om te testen gebruik leerlingnummer "999999" met wachtwoord "test123", voor docent: "mentor2024"', icon="ℹ️")
    tab_student, tab_teacher = st.tabs(["Leerling", "Docent"])

    with tab_student:
        lnr = st.text_input("Leerlingnummer", key="login_lnr", max_chars=10)

        if lnr:
            student = db.get_student(lnr)

            if student is None:
                st.error("Leerlingnummer niet gevonden. Vraag je mentor om je aan te melden.")

            elif not student["password_set"]:
                st.info("Eerste keer inloggen — stel een wachtwoord in.")
                pw1 = st.text_input("Nieuw wachtwoord", type="password", key="pw_new1")
                pw2 = st.text_input("Herhaal wachtwoord", type="password", key="pw_new2")
                if st.button("Wachtwoord instellen"):
                    if len(pw1) < 4:
                        st.error("Wachtwoord moet minimaal 4 tekens zijn.")
                    elif pw1 != pw2:
                        st.error("Wachtwoorden komen niet overeen.")
                    else:
                        db.set_password(lnr, pw1)
                        st.success("Wachtwoord ingesteld! Ververs de pagina en log in.")

            else:
                pw = st.text_input("Wachtwoord", type="password", key="login_pw")
                if st.button("Inloggen"):
                    result = db.verify_login(lnr, pw)
                    if result:
    ...
    with tab_teacher:
        dpw = st.text_input("Docentenwachtwoord", type="password", key="teacher_pw")
        if st.button("Inloggen als docent"):
            if dpw == db.TEACHER_PASSWORD:
                st.session_state.logged_in = True
...
if st.session_state.role == "student":
    ...
    with st.sidebar:
        ...
        st.caption(f"Mentorgroep: {user['mentorgroep']}")
        st.divider()

        completion = db.task_completion(lnr)
    ...
    # --- TASK 1: VERVOLGOPLEIDING ---
        ...
        with st.form("task1_form"):
            ...
            if st.form_submit_button("Opslaan", type="primary"):
                db.save_task1(lnr, {
                    "beoogd_beroep":    beroep,
                    "beoogde_opleiding": opleiding,
                    "stad":             stad,
                    "open_dag":         open_dag,
    ...
    # --- TASK 2: ORIËNTATIE ---
        ...
        st.info(
            "Maak eerst de beroepskeuzetest via de link hieronder. "
            "Vul daarna de velden in op basis van je uitkomsten."
        )
        st.markdown("**[Doe de beroepskeuzetest op 123test.nl](https://www.123test.nl/beroepskeuzetest/)**")
        st.divider()

        data = db.get_task("task2", lnr)
        with st.form("task2_form"):
            beroepen   = st.text_area(
                "Beroepen die bij mij passen (één per regel)",
                value=data.get("beroepen", ""), height=120,
            )
            opleidingen = st.text_area(
                "Opleidingen die ik interessant vind (één per regel)",
                value=data.get("opleidingen", ""), height=120,
            )
            reflectie = st.text_area(
                "Mijn reflectie — wat vond ik van de uitkomsten? Herken ik mezelf hierin?",
                value=data.get("reflectie", ""), height=160,
            )
    ...
    # --- TASK 3: OPEN DAGEN ---
        ...
        st.info(
            "Hier komt binnenkort een kalender met open dagen in de regio Amersfoort "
            "(Hogeschool Utrecht, HvA, Windesheim, ROC Midden Nederland e.a.). "
            "Noteer hieronder alvast je eigen initiatieven."
        )

        # Placeholder calendar info
        with st.expander("Bekende open dagen (placeholder — wordt uitgebreid)"):
            st.markdown("""
| School | Type | Datum | Inschrijven voor |
|--------|------|-------|-----------------|
| Hogeschool Utrecht | HBO | Nov 2024 | 1 mei |
| Windesheim Zwolle | HBO | Okt 2024 | 1 mei |
| ROC Midden Nederland | MBO | Doorlopend | — |
| Hogeschool van Amsterdam | HBO | Nov 2024 | 1 mei |
""")

        st.divider()
        data = db.get_task("task3", lnr)
        with st.form("task3_form"):
            notities = st.text_area(
                "Mijn open dag bezoeken — welke school, wanneer, wat vond je ervan?",
                value=data.get("notities", ""),
                height=220,
            )
            if st.form_submit_button("Opslaan", type="primary"):
                db.save_task3(lnr, {"notities": notities})
                st.success("Opgeslagen!")
                st.rerun()
    ...
    # --- TASK 4: INTERVIEWS ---
        ...
        st.write(
            "Interview drie mensen uit je omgeving over hun werk. "
            "Schrijf per persoon een verslag van **minimaal 300 woorden**."
        )
        ...
        VOORBEELD_VRAGEN = (
            ...
            "- Wat zijn de moeilijkste kanten van je baan?  \n"
            "- Welke opleiding heb je gedaan?  \n"
            "- Wat zou je anders doen als je opnieuw mocht kiezen?  \n"
            "- Wat voor eigenschappen heb je nodig in jouw beroep?"
        )

        interviews = db.get_task4(lnr)
        ...
        for nr in [1, 2, 3]:
            ...
            with st.expander(f"Interview {nr} — {naam_display}  {status}", expanded=not done):
                ...
                with st.form(f"task4_{nr}"):
                    ...
                    verslag = st.text_area(
                        "Jouw verslag",
                        value=reflectie_saved,
                        height=250,
                        help="Minimaal 300 woorden.",
                    )
                    wc_live = len(verslag.split()) if verslag.strip() else 0
                    kleur = "#27ae60" if wc_live >= 300 else "#e74c3c"
                    st.markdown(
                        f"<span style='color:{kleur};font-weight:bold;'>{wc_live} / 300 woorden</span>",
                        unsafe_allow_html=True,
                    )

                    if st.form_submit_button("Opslaan", type="primary"):
                        db.save_task4_interview(lnr, nr, {
                            "naam_persoon": naam,
                            "beroep":       beroep,
                            "relatie":      relatie,
                            "reflectie":    verslag,
...
# ============================================================
    ...
    with st.sidebar:
        st.markdown("**Docentendashboard**")
        st.divider()
        menu = st.radio("Weergave", ["Voortgang klas", "Leerling toevoegen"], label_visibility="collapsed")
        st.divider()
        if st.button("Uitloggen", use_container_width=True):
            logout()
    ...
    if menu == "Voortgang klas":
        ...
        for student in students:
            ...
            # Expandable detail per leerling
            with st.expander(f"Bekijk invullingen — {student['naam']}"):
                t1 = db.get_task("task1", lnr)
                t2 = db.get_task("task2", lnr)
                t3 = db.get_task("task3", lnr)
                t4 = db.get_task4(lnr)
                ...
                with dc1:
                    if t1:
                        ...
                        st.write(
                            f"Open dag: {'✅' if t1.get('open_dag') else '⬜'}  "
                            f"Meeloopdag: {'✅' if t1.get('meeloopdag') else '⬜'}  "
                            f"Ingeschreven: {'✅' if t1.get('ingeschreven') else '⬜'}"
                        )
                    else:
                ...
                with dc2:
                    if t2:
                        st.write("**Beroepen:**")
                        st.write(t2.get("beroepen") or "—")
                        st.write("**Opleidingen:**")
                        st.write(t2.get("opleidingen") or "—")
                        st.write("**Reflectie:**")
                        st.write(t2.get("reflectie") or "—")
                    else:
                        st.write("Nog niets ingevuld.")

                with dc3:
                    if t3:
                        st.write(t3.get("notities") or "—")
                    else:
                        st.write("Nog niets ingevuld.")
                ...
                with dc4:
                    if t4:
                        for nr, iv in sorted(t4.items()):
                            ...
                            st.markdown(
                                f"**Interview {nr} — {iv.get('naam_persoon', '?')} "
                                f"({iv.get('beroep', '?')}, {iv.get('relatie', '?')})** — {wc} woorden"
                            )
    ...
    elif menu == "Leerling toevoegen":
        st.title("Leerling toevoegen")

        with st.form("add_student_form"):
            lnr   = st.text_input("Leerlingnummer")
            naam  = st.text_input("Volledige naam")
            groep = st.text_input("Mentorgroep (bijv. 4H1)")

            if st.form_submit_button("Toevoegen", type="primary"):
                if lnr and naam and groep:
                    db.add_student(lnr, naam, groep)
                    st.success(f"{naam} toegevoegd. De leerling kan nu inloggen en een wachtwoord instellen.")
                else:
